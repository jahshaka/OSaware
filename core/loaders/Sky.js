...
Sky.SkyShader = {
	...
	fragmentShader: /* glsl */`
		...
		void main() {
			...
			// Self-contained ACES filmic tone-map + exposure 0.5 + sRGB, matching
			// the official example (renderer.toneMapping = ACESFilmic, exposure
			// 0.5). Baked in here so it does not depend on the shared r128
			// renderer's tone-mapping / colour-management settings.
			vec3 toned = texColor * 0.5;
			toned = clamp( ( toned * ( 2.51 * toned + 0.03 ) ) / ( toned * ( 2.43 * toned + 0.59 ) + 0.14 ), 0.0, 1.0 );
			gl_FragColor = vec4( pow( toned, vec3( 1.0 / 2.2 ) ), 1.0 );
